...
def clean_state():
    """Clear venue/race tables around every test so DB-backed list/count
    assertions are deterministic (venues are now DB metadata, not in-memory)."""
    def _clear():
        from app.database import SessionLocal
        from app.models import RaceResult, RaceEntry, Race, VenueRecord
        db = SessionLocal()
        try:
            db.query(RaceResult).delete()
            db.query(RaceEntry).delete()
            db.query(Race).delete()
            db.query(VenueRecord).delete()
            db.commit()
        finally:
            db.close()
    _clear()
...
def _setup_venue(venue_id="TESTTRACK") -> str:
    """Create a venue (metadata). Returns venue_id."""
